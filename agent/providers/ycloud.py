...
def _transcribir_audio(url: str, mime_type: str = "audio/ogg") -> str:
    """
    Descarga el audio de la URL y lo transcribe con Groq Whisper.
    Devuelve el texto transcripto, o None si falla.
    """
    groq_api_key = os.environ.get("GROQ_API_KEY", "")
    if not groq_api_key:
        print("[audio] GROQ_API_KEY no configurada — no se puede transcribir")
        return None
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"[audio] Error descargando audio: {resp.status_code}")
            return None
        ...
        print(f"[audio] Excepción al transcribir: {e}")
        return None
